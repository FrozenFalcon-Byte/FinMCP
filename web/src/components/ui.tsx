...
export function Ring({ pct, size = 84, stroke = 9, tone, children }: { pct: number; size?: number; stroke?: number; tone?: "warn" | "bad" | ""; children?: ReactNode }) {
  ...
  return (
    // The size arrives as a variable rather than a width, so a cramped card can cap it in CSS (see `--ring-fit`).
    <div className={`ring ${t}`} style={{ "--ring": `${size}px` } as React.CSSProperties}>
...
export type IconName =
  ...
  | "chart" | "crop" | "zoomIn" | "zoomOut" | "rotate";
...
export function Icon({ name, className }: { name: IconName; className?: string }) {
  ...
  switch (name) {
    ...
    case "chart": return <svg {...common}><path d="M4 20V4M4 20h16" /><path d="M8 20v-6M12.5 20V8M17 20v-9" /></svg>;
    case "crop": return <svg {...common}><path d="M6.5 2.5v15h15M2.5 6.5h15v15" /></svg>;
    case "zoomIn": return <svg {...common}><circle cx="11" cy="11" r="6.5" /><path d="M20 20l-4.2-4.2M11 8.5v5M8.5 11h5" /></svg>;
    case "zoomOut": return <svg {...common}><circle cx="11" cy="11" r="6.5" /><path d="M20 20l-4.2-4.2M8.5 11h5" /></svg>;
    case "rotate": return <svg {...common}><path d="M4 12a8 8 0 1 0 2.3-5.7M4 4v5h5" /></svg>;
